...
//    jfieldId fid = (*env)->GetFieldID(clazz, "nativeStruct", "J");
    (env)->SetLongField(obj, (env)->GetFieldID(clazz, "nativeStruct", "J"), (jlong) ptr);
}
...
JNIEXPORT void JNICALL Java_edu_sju_ee98_ni_daqmx_analog_ContGenIntClk_destruct
  (JNIEnv *env, jobject obj){
    ...
    struct data *ptr = (struct data *) ((env)->GetLongField(obj, (env)->GetFieldID((env)->GetObjectClass(obj), "nativeStruct", "J")));
    free(ptr);
}
...
//    TaskHandle taskHandle = 0;
    ...
    DAQmxErrChk(DAQmxCreateAOVoltageChan(ptr->taskHandle, (env)->GetStringUTFChars(physicalChannel, 0), "", minVoltage, maxVoltage, DAQmx_Val_Volts, NULL));
    DAQmxErrChk(DAQmxCfgSampClkTiming(ptr->taskHandle, "", rate, DAQmx_Val_Rising, DAQmx_Val_ContSamps, length));

    DAQmxErrChk(DAQmxRegisterDoneEvent(ptr->taskHandle, 0, DoneCallback, NULL));

    /*********************************************/
    // DAQmx Write Code
    /*********************************************/
    DAQmxErrChk(DAQmxWriteAnalogF64(ptr->taskHandle, length, 0, 10.0, DAQmx_Val_GroupByChannel, (env)->GetDoubleArrayElements(data, JNI_FALSE), NULL, NULL));
...
Error:
    ...
    if (ptr->taskHandle != 0) {
        ...
        // DAQmx Stop Code
        /*********************************************/
        DAQmxStopTask(ptr->taskHandle);
        DAQmxClearTask(ptr->taskHandle);
    }
    if (DAQmxFailed(error))
